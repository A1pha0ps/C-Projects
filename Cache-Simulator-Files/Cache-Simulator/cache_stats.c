...
void calculate_stat_rates(cache_stats_t *stats, int block_size)
{
  ...
  stats->B_cache_to_bus_wt = stats->n_stores * NUMBYTES_PER_STORE;
  stats->B_total_traffic_wb = stats->B_cache_to_bus_wb + stats->B_bus_to_cache;
  stats->B_total_traffic_wt = stats->B_cache_to_bus_wt + stats->B_bus_to_cache;
}
